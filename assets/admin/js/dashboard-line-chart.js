...
var DashboardLines = function() {
    ...
    var _EnquiryLineChart = function(element, height) {
        ...
        if(element) {
            ...
            d3.json('/admin/dashboard/graph/enquiry', function(error, data) {
                formatted = data;
                redraw();
            });
            ...
            function redraw() {
                ...
                color.domain(d3.keys(data[0]).filter(function (key) { return (key !== 'log_date' && key !== 'product_name'); }));
                ...
                var linedata = color.domain().map(function(name) {
                    return {
                                ...
                                values: data.map(function(d) {
                                    return {name: name, date: parseDate(d.log_date), value: parseFloat(d[name], 10)};
                                })
                            };
                        });
                ...
                var x = d3.time.scale()
                    .domain([
                        d3.min(linedata, function (c) { return d3.min(c.values, function (v) { return v.log_date; }); }),
                        d3.max(linedata, function (c) { return d3.max(c.values, function (v) { return v.log_date; }); })
                    ])
                    .range([0, width]);
                ...
                var circles = lines.selectAll('circle')
                    ...
                    .attr('cx', function (d, i) { return x(d.log_date)})
                ...
                function appSalesResize() {
                    ...
                    // Circles
                    svg.selectAll('.d3-line-circle').attr('cx', function (d, i) { return x(d.log_date)})
                }
            }
        }
    };
    ...
    var _ReviewLineChart = function(element, height) {
        ...
        if(element) {
            ...
            var tooltip = d3.tip()
                ...
                .html(function (d) {
                    return '<ul class="list-unstyled mb-1">' +
                        '<li>' + '<div class="font-size-base my-1"><i class="icon-check2 mr-2"></i>' + formatDate(d.log_date) + '</div>' + '</li>' +
    ...
    var _UsedLineChart = function (element, height) {
        if (typeof d3 == 'undefined') {
            console.warn('Warning - d3.min.js is not loaded.');
            return;
        }
    ...
    return {
        init: function() {
            _EnquiryLineChart('#enquiry_chart', 255);
            _ReviewLineChart('#review_chart', 255);
            _UsedLineChart('#used_chart', 255);
        }
    }
}();
